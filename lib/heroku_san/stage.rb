...
module HerokuSan
  class Stage
    ...
    def maintenance(action = nil)
      if block_given?
        heroku.post_app_maintenance(app, true)
        begin
          yield
        ensure
          heroku.post_app_maintenance(app, false)
        end
      else
        raise ArgumentError, "Action #{action.inspect} must be one of (:on, :off)", caller if ![:on, :off].include?(action)
        heroku.post_app_maintenance(app, {:on => true, :off => false}[action])
      end
    end
    ...
    def restart
      "restarted" if heroku.dyno.restart_all(app) == {}
    end
